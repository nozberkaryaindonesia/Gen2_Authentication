...
def main():
    ...
    while 1:
        ...
        for k in range(0, i):
            ...
            (RN16_T, RN16_R) = print_log_msg(msg, log_file, RN16_T, RN16_R)
...
def print_log_msg(msg, log_file, RN16_T, RN16_R):
    ...
    if msg.type() == LOG_ACK:
        fields = split(strip(msg.to_string()), " ")
        print "%s\t R=>T \t%s RN16_R: %04X%s" %(fields[-1], fOrange, int(fields[0],2), fReset)
        RN16_R = int(fields[0],2)
        log_file.write(fields[-1] + ",ACK,0,0,0\n");

    if msg.type() == LOG_NAK:
        fields = split(strip(msg.to_string()), " ")
        #print "%s\t R=>T \t NAK" %(fields[-1])
    ...
    if msg.type() == LOG_RN16:
        ...
        if msg.arg2() == LOG_ERROR:
            ...
            print "%s\t R<=T \t%s RN16_T w/ Error: %04X%s" %(fields[-1],fRed, tmp, fReset)
            log_file.write(fields[-1] + ",RN16,1," +"%04X" % tmp  + ","+snr + "\n");
            RN16_T = tmp
        else:
            print "%s\t R<=T \t%s RN16_T: %04X%s" %(fields[-1],fBlue, tmp, fReset)
    ...
    if msg.type() == LOG_EPC:
        ...
        tmp = int(epc,2)
        #the reader side verfication is ommitted, we just simply check the CRC
        if msg.arg2() == LOG_ERROR:
            print "%s\t R=>T \t%s EPC: %024X%s (verification failed!)\n\n" %(fields[-1],fRed, tmp, fReset)
            log_file.write(fields[-1] + ",EPC,1," + "%024X" % tmp + ","+snr + "\n");
        else:
            print "%s\t R=>T \t%s EPC: %024X%s (verification succeeded!)\n\n" %(fields[-1],fGreen, tmp, fReset)
