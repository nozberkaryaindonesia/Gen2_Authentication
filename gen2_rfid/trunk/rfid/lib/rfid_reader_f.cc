...
rfid_reader_f::send_ack(){
    ...
    //generate a random number for authentication purpose
    char reader_nounce[16];
    if(auth_flag == false){
        for(int i = 0; i < 14; ++i){
            int tmp = rand()%2;
            reader_nounce[i] = 48+tmp;
        }
    }
    else{
        for(int i = 0; i < 14; ++i)
    ...
    reader_nounce[14] = '0';
    reader_nounce[15] = '1';
    int i_reader_nounce = 0x00;
    for(int i = 0; i < 16; ++i)
    i_reader_nounce |= (reader_nounce[i]-48) << (15-i);
    
    //printf("reader nounce: \t %4x\n", i_reader_nounce);
    
    log_msg(LOG_ACK, reader_nounce, LOG_OKAY);
    //printf("LOG_ACK\n");
    ...
    for(int i = 0; i < global_reader_state->num_bits_to_decode - 1; i++){
        ...
        //payload of the ACK cmd is the RN16 received
        //replace this RN16 with another RN16 for authentication purpose
        //if(global_reader_state->tag_bit_vector[i] == '0'){
            if(reader_nounce[i] == '0'){
